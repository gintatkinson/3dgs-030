...
class _PropertyGridState extends State<PropertyGrid> {
  ...
  void _initializeFields(List<FieldDescriptor> fields, Map<String, dynamic> nodeData) {
    for (final field in fields) {
      ...
      if (field.type != 'enum' && field.type != 'dateTime') {
  ...
  (bool isValid, dynamic parsedValue, String? error) _validateField(
    ...
    if (field.type == 'double') {
      ...
      final error = validateIso8601(valueString);
      if (error != null && valueString.isNotEmpty) {
        return (false, valueString, error);
      }
  ...
  Widget _buildAttrField(FieldDescriptor field, bool isDark, double panelOpacity) {
    ...
    if (field.readOnly) {
      final value = committedData[field.key]?.toString() ??
    ...
    if (field.type == 'enum') {
      ...
      return DateTimePickerField(
        ...
        onChanged: (value) {
          ...
          setState(() {
            committedData[field.key] = value;
          });
        },
        validator: validateIso8601,
      );
    } else {
